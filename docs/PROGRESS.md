...
마지막 갱신: 2026-08-05, 커밋 `cff6edc`(Phase 5)까지 반영.
...
```
마지막 실행 결과(2026-08-05, Phase 5 후): assembleDebug 성공, 유닛테스트 39/39 통과, lint 차단오류 0건(경미한 경고 7건). rules 테스트는 Phase 3에서 12/12 통과(Phase 4·5는 rules 변경 없음 — 기존 규칙이 서브컬렉션 CRUD를 이미 커버).
...
- **미검증**: 바우처 실제 파일 첨부 플로우(SAF 문서 피커 UI 자동화가 불안정해 수동 확인 필요. 빈 상태 표시·추가 버튼·검증 코드 경로는 존재)
...
docs/PROGRESS.md 와 docs/SPEC.md 를 먼저 읽고, Phase 6(주변 추천·위치 권한·Haversine·
추천 점수·Maps Intent)부터 같은 방식(Demo/Firebase 이중 구현, 실제 빌드·테스트·에뮬레이터 검증,
