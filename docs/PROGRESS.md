...
이것으로 Part 2(로그아웃·회원 탈퇴·개인정보처리방침·이용약관 초안·release 서명
키스토어) 전부 완료. 회원 탈퇴를 실제 계정으로 끝까지 실행하는 검증은 되돌릴 수
없는 데이터 삭제라 이번엔 보류(코드·규칙 테스트로만 검증됨) — 필요해지면 별도로 진행.

**추가 기능(사용자 요청, Part 2 계획 밖)**: 여행 이름·기간·기본통화 수정 UI를
전체 탭 여행 정보 화면에 추가(소유자 + 완료되지 않은 여행에서만, 도시 수정과
동일한 게이팅). `firestore.rules`는 이미 소유자의 임의 필드 변경을 허용하고
있어 규칙 변경 불필요, 회귀 테스트 2건 추가(최종 36/36 통과). 이 기능으로 이전
세션의 adb 자동 입력 실수로 깨져 있던 실제 테스트 여행 이름("ㅆㄷㄴㅅㅆ갸ㅔ")을
실기기에서 직접 "test"로 수정해 실제 Firestore 쓰기까지 확인.

## 배포 후 실기기 QA (v0.1.1 ~ v0.1.3, 2026-08-11)

APK를 GitHub Releases에 올리고 `wjdals988/mydashboard`(coldbrewventi.vercel.app)가
URL·크기·SHA-256을 참조하는 배포 경로를 세운 뒤, release 서명 + R8 난독화 상태의
실기기(Galaxy SM-S948N)에서 5개 탭 전체를 직접 조작해 검증했다. 에뮬레이터·debug
빌드에서는 드러나지 않던 문제가 여기서 3종 나왔다.

**v0.1.1 — Google 로그인이 아예 되지 않던 문제.** 로그인 버튼을 눌러도 로딩만 돌고
아무 일도 없었다. 원인이 두 겹이었다. (1) release 키스토어 SHA-1이 Firebase에
등록돼 있지 않아 Google이 자격 증명을 하나도 돌려주지 않았고(그런데 앱은 "기기에
등록된 계정이 없습니다"라는 엉뚱한 안내를 띄우고 있었다 — 두 원인을 함께 알려주도록
문구를 고쳤다), (2) 지문을 등록한 뒤에도 `GetGoogleIdOption`의 자동 로그인 바텀시트가
이 기기에서 투명한 빈 창으로 떠 `getCredential`이 영구히 반환되지 않았다(호출 14회·
완료 0회, `CredentialSelectorActivity`가 resumed인데 렌더링 없음). 명시적 버튼용
옵션인 `GetSignInWithGoogleOption`으로 교체해 해결. R8은 무죄였다(난독화 없는 debug
빌드에서도 동일 재현). 함께 발견한 결함 2건: `AuthGate`의 오류 콜백이 빈 람다여서
Firebase 인증 실패가 조용히 사라졌고, 버튼 연타 시 새 요청이 앞선 요청의 선택창을
계속 취소시켜 어떤 요청도 완료되지 않는 교착이 있었다.

**v0.1.2 — 완료된 여행에서 쓰기 시 앱이 죽던 크래시.** 여행을 완료 처리한 뒤 일정
카드 ⋮ → "완료로 표시"를 누르면 `PERMISSION_DENIED`로 앱이 즉시 종료됐다. 서버 규칙이
완료된 여행의 쓰기를 거부하는데 `viewModelScope.launch` 안에서 그 예외를 잡지 않은
것이 원인이고, 예전에 `TripInfoViewModel`에서 고친 것과 같은 패턴이 나머지 ViewModel
전부에 남아 있었다. `core/error/ActionErrorState.kt`(실패 → 사용자 메시지 + Firestore
영문 원문 차단) + `core/ui/ActionErrorSnackbar.kt`를 만들어 목록·편집기 전체에 일괄
적용했다. 편집기는 실패 시 화면을 닫지 않고 이유를 띄운다(예전에는 저장 성공을 전제로
`onSaved()`를 호출했다). 함께 고친 것: 읽기전용 게이팅이 FAB에만 걸려 있어 행 단위
⋮ 메뉴·수정·삭제 버튼으로 크래시에 도달할 수 있었고, 루트 Scaffold와 하위 화면의
TopAppBar가 상태표시줄 여백을 각각 적용해 제목 위 공간이 두 배로 벌어져 있었다
(`NavHost`에 `consumeWindowInsets`).

UI를 가리는 것만으로 끝내지 않은 이유: 파트너가 먼저 여행을 완료 처리하면 내 화면은
아직 옛 상태여서 버튼이 살아 있고, 그 사이 누르면 같은 예외가 난다. 이 경합은 기기
2대가 필요해 실기기로 재현하지 못했고 유닛테스트로만 검증했다(남은 리스크).

**v0.1.3 — 사용자 요청 3건.** (1) 도시 체류 시작일·종료일 입력 UI. 이전까지
`CityFormDialog`이 이름·국가코드·시간대만 받아, `HomeViewModel`의 "현재 도시 기준
현지 시각" 경로(도시의 체류 기간이 오늘을 포함해야 발동)가 UI로는 절대 발동할 수 없는
죽은 코드였다. 스위치로 켜는 선택 입력으로 넣고 두 날짜를 항상 함께 저장한다(한쪽만
있으면 기간 판정이 불가능). 실기기에서 Prague에 오늘 날짜를 넣어 홈이 "현지 13:19 ·
한국 20:19"로 바뀌는 것까지 확인. (2) 손글씨체(Gaegu)를 시스템 기본 폰트로 되돌리고
폰트 파일 6MB도 삭제(위 Part 1 타이포 항목은 그 시점의 결정 기록으로 남겨둔다).
(3) 전체 탭에 "버전 정보" 화면 추가 — `BuildConfig`에서 읽은 설치 버전과
`core/version/ReleaseHistory.kt`의 버전별 변경 내역을 함께 보여준다. 스토어를 거치지
않는 배포라 "무엇이 바뀌었는지" 볼 곳이 없었고, 기내·로밍 환경을 감안해 원격 조회가
아니라 앱 내 상수로 둔다.
